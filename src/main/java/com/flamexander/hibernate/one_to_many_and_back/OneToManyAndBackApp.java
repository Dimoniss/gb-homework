...
//            session.getTransaction().commit();
            ...
            final Serializable id = session.save(newUniversity);
            session.getTransaction().commit();

            session = factory.getCurrentSession();
            session.beginTransaction();
            final University university = session.get(University.class, id);
            university.setTitle("newTitle");
            session.getTransaction().commit();

            session = factory.getCurrentSession();
            session.beginTransaction();
            final University university1 = session.get(University.class, id);
            session.getTransaction().commit();
